...
public class ADB {
    ...
    public String getForegroundActivity() {
        return command("adb -s " + ID + " shell dympsys window windows | grep mCurrentFocus");
    }

    public String getAndroidVersionAsString() {
        String output = command("adb -s " + ID + " shel getprop ro.build.version.release");
    ...
    public Object startLogcat(String logID, String grep) {
        ...
        ArrayList pidsAfter = getLogcatProcesses();
        Timer timer = new Timer();
        timer.start();
        while (!timer.expired(5)) {
            if (pidBefore.size() > 0) pidsAfter.removeAll(pidBefore);
            if (pidsAfter.size() > 0) break;
            pidsAfter = getLogcatProcesses();
        }
        if (pidsAfter.size() == 1) return pidsAfter.get(0);
        else if (pidsAfter.size() > 1)
            throw new RuntimeException("Multiple logcat processes were started when only one expected!");
